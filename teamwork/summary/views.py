...
def add_status(request, user_id, contest_id):
    ...
    if request.method != 'POST':
        form = StatusForm(contest_id = contest_id)
    else:
        form = StatusForm(contest_id = contest_id, data = request.POST)
        if form.is_valid():
            ...
            for key,value in form.cleaned_data.items():
                if key[0]=='p':
                    t.append(value)
            status.ac_status = strlist_to_int(t)
            status.save()
